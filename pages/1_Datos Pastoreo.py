...
st.subheader("🎯 KPIs de Aforo - Vista Rotativa")

# Obtener lista de fincas
fincas = dfpasto["FINCA"].unique().tolist()

# Crear placeholder
placeholder = st.empty()

# Loop infinito que rota entre fincas
if 'finca_index' not in st.session_state:
    st.session_state.finca_index = 0

# Botón para pausar/reanudar
col_pause, col_manual = st.columns([1, 4])
with col_pause:
    auto_rotate = st.checkbox("Auto-rotar", value=True)

# Obtener finca actual
finca_actual = fincas[st.session_state.finca_index]
data_finca = dfpasto[dfpasto["FINCA"] == finca_actual].copy()

# Calcular métricas
if len(data_finca) >= 20:
    aforo_actual = data_finca["AFORO PLATOMETRO (Kg/m2)"].iloc[-10:].mean()
    aforo_anterior = data_finca["AFORO PLATOMETRO (Kg/m2)"].iloc[-20:-10].mean()
    delta_aforo = ((aforo_actual - aforo_anterior) / aforo_anterior * 100) if aforo_anterior > 0 else 0
    
    consumo_actual = data_finca["CONSUMO PASTO PLATOMETRO (Kg/vaca/día)"].iloc[-10:].mean()
    consumo_anterior = data_finca["CONSUMO PASTO PLATOMETRO (Kg/vaca/día)"].iloc[-20:-10].mean()
    delta_consumo = ((consumo_actual - consumo_anterior) / consumo_anterior * 100) if consumo_anterior > 0 else 0
    
    dias_rotacion = data_finca['DIAS ROTACION'].mean()
else:
    aforo_actual = data_finca["AFORO PLATOMETRO (Kg/m2)"].mean()
    delta_aforo = 0
    consumo_actual = data_finca["CONSUMO PASTO PLATOMETRO (Kg/vaca/día)"].mean()
    delta_consumo = 0
    dias_rotacion = data_finca['DIAS ROTACION'].mean()

# Mostrar con placeholder
with placeholder.container():
    st.markdown(f"### 🏢 {finca_actual}")
    ...
    with col1:
        st.metric(
            ...
            delta=f"{delta_aforo:.1f}%"
        )
    
    with col2:
        st.metric(
            label="🐄 Consumo Promedio",
            value=f"{consumo_actual:.1f} Kg/vaca/día",
            delta=f"{delta_consumo:.1f}%"
        )
    
    with col3:
        st.metric(
            label="📅 Días Rotación",
            value=f"{dias_rotacion:.0f} días"
        )
    
    # Gráfica de tendencia para la finca actual
    data_finca["MES_ANO"] = data_finca["MES_ANO"].astype(str)
    data_agrupado = data_finca.groupby("MES_ANO")["AFORO PLATOMETRO (Kg/m2)"].mean().reset_index()
    data_agrupado = data_agrupado.sort_values("MES_ANO")
    
    fig = px.line(data_agrupado,
                  x="MES_ANO",
                  y="AFORO PLATOMETRO (Kg/m2)",
                  title=f"Tendencia de Aforo - {finca_actual}",
                  markers=True)
    ...
    progress_text = f"Finca {st.session_state.finca_index + 1} de {len(fincas)}"
    st.progress((st.session_state.finca_index + 1) / len(fincas), text=progress_text)

# Auto-rotar cada 5 segundos
if auto_rotate:
    time.sleep(5)
    st.session_state.finca_index = (st.session_state.finca_index + 1) % len(fincas)
    st.rerun()
